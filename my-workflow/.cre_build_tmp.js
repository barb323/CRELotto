...
var writeDataOnchain = (runtime2) => {
  const zahl = BigInt(runtime2.config.zahl);
  ...
  const evmClient = new ClientCapability(network295.chainSelector.selector);
  const reportData = encodeAbiParameters(parseAbiParameters("uint256"), [zahl]);
  runtime2.log(`Encoded data for consumer contract`);
